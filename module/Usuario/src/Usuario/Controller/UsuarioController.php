...
class UsuarioController extends AbstractActionController
{
    ...
    public function validaCampos($datos, $action){
        ...
        if( $action!="MISDATOS" ){
            if(!is_numeric($datos["tipo"]) or (int)$datos["tipo"]<=0 ){
                return "Debe seleccionar un tipo de usuario válido.";
            }

            //SI ES ADMIN O LOCUTOR
            if( (int)$datos["tipo"]==2 || (int)$datos["tipo"]==3 ){

                if( !isset($datos["radio"]) || !is_numeric($datos["radio"]) || (int)$datos["radio"]<=0 ){
                    return "Debe seleccionar una radio para el usuario.";
                }
            }
        }
    ...
    public function editarAction()
    {
        ...
        //OBTENEMOS LA RADIO
        $radio=null;
        if( ($usuario['tipo_user'] == 2 || $usuario['tipo_user'] == 3) && !empty($usuario['idr']) ){
            try {
                $radio = $modeloRadios->get( $usuario['idr'] );
            } catch (\Exception $e) {}
        }
        //****************************************************************
