...
class TablausuariosController extends AbstractActionController
{
    ...
    public function listaAction()
    {
        ...
        if(!empty($idr)){
            if(!empty($sWhere))
                $sWhere.=" AND u.idr='".$idr."' ";
            else
                $sWhere.=" WHERE u.idr='".$idr."' ";
        }
        ...
        $sQuery = "
            ...
            r.radio_name
            FROM usuarios u
            INNER JOIN usuarios_profiles AS up ON u.tipo_user=up.idprof
            LEFT JOIN radios AS r ON u.idr=r.idr
            $sWhere
            $sOrder
            $sLimit
            ";
        ...
        foreach ($rResult as $aRow) 
        {
            ...
            for ( $i=0 ; $i<count($aColumns) ; $i++ )
            {
                ...
                {
                    //MOSTRAMOS RADIO
                    $extra_radio="<br /><i class='fa fa-microphone'></i> Radio: -- No aplica --";
                    ...
                    $row["nombre"]=stripslashes($aRow['nombre_completo']).'<br />'.$extra_radio;
                }
