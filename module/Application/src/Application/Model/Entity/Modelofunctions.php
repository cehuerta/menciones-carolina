...
class Modelofunctions{
    ...
    public function current_timezone_form_epoch( $type, $timezone=0, $epoch_time=0 ) {

        switch ( $type ) {
            case 'mysql':
                if( $epoch_time!==0 )
                    return ( $timezone===0 )? gmdate( 'Y-m-d H:i:s') : gmdate( 'Y-m-d H:i:s', ( $epoch_time + ( $timezone ) ) );
                else
                    return ( $timezone===0 )? gmdate( 'Y-m-d H:i:s') : gmdate( 'Y-m-d H:i:s', ( time() + ( $timezone ) ) );

            case 'timestamp':
                if( $epoch_time!==0 )
                    return ( $timezone===0 )? $epoch_time : $epoch_time + ( $timezone );
                else
                    return ( $timezone===0 )? time() : time() + ( $timezone );

            default:

                if( $epoch_time!==0 )
                    return ( $timezone===0 )? gmdate($type) : gmdate( $type, ($epoch_time + ( $timezone )) );
                else
                    return ( $timezone===0 )? gmdate($type) : gmdate( $type, (time() + ( $timezone )) );
        }
    }
