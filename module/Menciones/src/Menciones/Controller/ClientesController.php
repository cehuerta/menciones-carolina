...
class ClientesController extends AbstractActionController
{
    ...
    public function clientesAction()
    {
        ...
        $radios = array();
        if( ((int)$identidad->tipo_user==1) || ((int)$identidad->tipo_user==2) ){
            try {
    ...
    public function autocompleteAction()
    {
        ...
        $autocomplete_tmp=$modeloCliente->all(0, 10, $client_name, 'c.client_name ASC', null, $idr );//START, LIMIT, NAME
        ...
        if ( $autocomplete_tmp!=NULL){
            foreach ($autocomplete_tmp as $value) {
                ...
                $autocomplete[]=array(
                    'nombre'        => stripslashes($value["client_name"]).' - ['.stripslashes($value["radio_name"]).']',
                    'id'            => $value["idc"]
                );
            }
        }
