...
class TablacampanasController extends AbstractActionController
{
    ...
    public function listaAction()
    {
        ...
        try {
            $rResult = $modeloCampana->all( $sStart, $sLimit, null, null, null, $sOrder, $sWhereLike, $idr );
        } catch (\Exception $e) {
            $output = array(
                "sEcho"                 => abs($_GET["sEcho"]),
                "iTotalRecords"         => 0,
        ...
        foreach ($rResult as $aRow) 
        {
            ...
            for ( $i=0 ; $i<count($aColumns) ; $i++ )
            {
                ...
                else if ( $aColumns[$i] == 'campaign_title' )
                {
                    ...
                    //MOSTRAMOS NOMBRE
                    $row["nombre"] = '<i class="fa fa-th-list" ></i> <a href="'.$url_detalle.'" class="text-bold btn-link">'.stripslashes($aRow["campaign_title"]).'</a><br />'.$extra_t_menciones.$extra_radio;
                }
