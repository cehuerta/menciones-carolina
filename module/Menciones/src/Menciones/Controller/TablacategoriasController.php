...
class TablacategoriasController extends AbstractActionController
{
	public $dbAdapter;
    private $auth;
    public $rutas;
    ...
    public function listaAction()
    {
        ...
        $aColumns = array( 'category_name', 'category_date_create', 'version');
        // $idr      = null;
        ...
        try {
            $rResult = $modeloCategoria->all( $sStart, $sLimit, null, $sOrder, $sWhereLike);
        } catch (\Exception $e) {
            $output = array(
                "sEcho"                 => abs($_GET["sEcho"]),
                "iTotalRecords"         => 0,
        ...
        try {
            $iFilteredTotal = abs($modeloCategoria->found());
        } catch (\Exception $e) {
            $output = array(
                "sEcho"                 => abs($_GET["sEcho"]),
                "iTotalRecords"         => 0,
        ...
        $url_get="?categoria=".$search."&length=".$length."&page=".$page."&order=".$order.'&order_dir='.$order_dir;

        foreach ($rResult as $aRow) 
        {
            $row = array();
            $row["DT_RowId"]="row_".$aRow["idca"];
            ...
            for ( $i=0 ; $i<count($aColumns) ; $i++ )
            {
                if ( $aColumns[$i] == "version" )
                {
                    ...
                    $var="<div class='btnsAccion'>";
                        ...
                        $var.="<button class='btn btn-default btn-sm linkEditar' data-toggle='tooltip' data-placement='top' title='Editar' data='".$aRow["idca"]."'><span class='fa fa-wrench'></span></button>";
                        $var.="<button class='btn btn-danger btn-sm linkEliminar' data-toggle='tooltip' data-placement='top' title='Eliminar' data='".$aRow["idca"]."'><span class='fa fa-remove'></span></button>";

                    $var.="</div>";
                    $row["version"]=$var;
                }
                else if ( $aColumns[$i] == 'category_date_create' )
                {
                    $row["fecha"] = $aRow["category_date_create_fecha"].' | '.$aRow["category_date_create_hora"];
                }
                else if ( $aColumns[$i] == 'category_name' )
                {
                    ...
                    //MOSTRAMOS NOMBRE
                    $row["nombre"] = '<i class="fa fa-user" ></i> <span class="text-info text-semibold">'.stripslashes($aRow["client_name"]).'</span><br />'.$extra_t_campanas.$extra_radio;
                }
