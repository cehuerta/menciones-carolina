...
class Modelodashboard{
    ...
    public function totalMenciones($dia=null, $dia_num=null, $mention_status=null){
    ...
    public function totalCampanas($campaign_status=null){
    ...
    public function totalUsuarios($tipo_user=null){
    ...
    public function readLastDays($id_user=null){
        ...
        $select->from( 'menciones_vistas' );

        $select->where('mention_view_date >= DATE_ADD(NOW(), INTERVAL -7 DAY)');

        if($id_user!==null)
            $select->where(array('id_user'=>$id_user));
        
        $select->having('total>0');
        $select->group( new Expression('date_format(mention_view_date,"%d-%m-%Y")') );
        $select->limit(10);
